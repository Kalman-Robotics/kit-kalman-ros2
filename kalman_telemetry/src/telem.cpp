...
class KalmanTelemetry : public rclcpp::Node
{
public:
  inline static const std::string NODE_NAME = "kalman_telemetry_node";
...
private:
  ...
  static int read_byte_callback(const void * context)
  {
    ...
    return reinterpret_cast<KalmanTelemetry*>(ctx)->read_lds_byte();
  }
  ...
  {
    void * ctx = const_cast<void *>(context);
    return reinterpret_cast<KalmanTelemetry*>(ctx)->process_scan_point(
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<KalmanTelemetry>());
  rclcpp::shutdown();
  return 0;
}
